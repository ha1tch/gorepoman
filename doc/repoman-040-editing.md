...
```
$ python3 ed.py find maxRetries server.go
...
```
$ python3 ed.py apply server.go:34-44:73022d1a --with maxConnectRetries
...
```
$ python3 ed.py sub "connecting to" "dialing" server.go --expect 1
  server.go: 1
...
```
$ python3 ed.py mark before_rename
mark 'before_rename' at txn 1
$ python3 ed.py sub "connecting to" "dialing" server.go --expect 1
  server.go: 1
replaced 1 occurrence(s) across 1 file(s)
$ python3 ed.py undo --since before_rename
...
```
$ python3 ed.py find "connecting to" server.go
server.go:105-118:8fa86ff8  [go-dquote-string]  line 8: fmt.Println("connecting to", host)
1 occurrence(s)

$ python3 ed.py sub "func connect" "func Connect" server.go --expect 1
  server.go: 1
replaced 1 occurrence(s) across 1 file(s)

$ python3 ed.py apply server.go:105-118:8fa86ff8 --with "reaching out to"
...
`str_replace_extended.py` never accepts search or replace text as CLI
arguments — only as a base64-encoded JSON payload (`v: 1`, a list of
`ops`), fed via stdin, a file, or, most usefully, by importing
`apply_payload()` directly as a library rather than shelling out:
...
`python-code` twice, `python-comment` once. Asserting `roles:
...
`timeout` left in the file) and touches that. Result:
